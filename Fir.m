...
b_hpf = fir1(order_fir, 0.5/(fs/2), 'high',hamming(order_fir+1));
a_hpf=1;

% 50 Hz Notch filter
b_notch = fir1(order_fir,[49 51]/(fs/2),'stop',hamming(order_fir+1));
a_notch=1;

% Low-pass FIR filter
b_lpf= fir1(order_fir,100/(fs/2), 'low',hamming(order_fir+1));
a_lpf=1;
...
title('Original vs FIR Filtered ECG');
...
title('FIR of High pass Filter Frequency Response');
...
title('FIR Low pass of Filter Frequency Response');
...
xlabel('Time in s');
ylabel('Amplitude');
title('Impulse Response - FIR High pass filter');
...
xlabel('Time in s');
ylabel('Amplitude');
title('Impulse Response - FIR Low pass filter');
...
xlabel('Time in s');
ylabel('Amplitude');
title('Step Response of FIR High pass filter');
...
xlabel('Time (s)');
ylabel('Amplitude');
title('Step Response - FIR Low pass filter');

% Pole-Zero Plots
figure;
zplane(b_hpf, a_hpf);
title('Pole-Zero Plot - FIR High pass filter');
...
figure;
zplane(b_lpf, a_lpf);
title('Pole-Zero Plot - FIR Low pass filter');

%display coefficients of numerator and denominator of Butterworth

disp('FIR High-pass numerator coefficient b_hpf:');
disp(b_hpf);

disp('FIR High pass filter denominator coefficient of a_hpf:');
disp(a_hpf);
...
disp('FIR Low-pass numerator coefficient of b_lpf:');
disp(b_lpf);

disp('FIR Low-pass denominator coefficient of a_lpf:');
disp(a_lpf);
