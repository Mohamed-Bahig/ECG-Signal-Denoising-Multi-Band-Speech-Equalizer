...
[b_hpf, a_hpf] = butter(4, 0.5/(fs/2), 'high');
...
[b_lpf, a_lpf] = butter(4, 100/(fs/2), 'low');
...
title('Original vs Butterworth Filtered ECG');
...
title('butterworth of High pass Filter Frequency Response');
...
title('butterworth Low pass of Filter Frequency Response');
...
xlabel('Time in s');
ylabel('Amplitude');
title('Impulse Response - Butterworth High pass filter');
...
xlabel('Time in s');
ylabel('Amplitude');
title('Impulse Response - Butterworth Low pass filter');
...
xlabel('Time (s)');
ylabel('Amplitude');
title('Step Response - butterworth Low pass filter');

% Pole-Zero Plots
figure;
zplane(b_hpf, a_hpf);
title('Pole-Zero Plot - butterworth High pass filter');
...
figure;
zplane(b_lpf, a_lpf);
title('Pole-Zero Plot - butterworth Low pass filter');

%display coefficients of numerator and denominator of Butterworth

disp('Butterworth High-pass numerator coefficient b_hpf:');
disp(b_hpf);

disp('Butterworth High pass filter denominator coefficient of a_hpf:');
disp(a_hpf);
...
disp('Butterworth Low-pass numerator coefficient of b_lpf:');
disp(b_lpf);

disp('Butterworth Low-pass denominator coefficient of a_lpf:');
disp(a_lpf);
